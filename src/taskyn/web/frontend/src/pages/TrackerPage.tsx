...
export function TrackerPage() {
  ...
  const handleManualEntry = async () => {
    ...
    logTime.mutate(
      { node_id: manualNodeId, duration_minutes: parseInt(manualDuration, 10), notes: manualNotes.trim() || undefined },
      { onSuccess: () => { setShowManual(false); setManualNodeId(''); setManualDuration('60'); setManualNotes(''); } },
    );
  };

  const handleDeleteEntry = async () => {
    if (!deleteEntryId) return;
    deleteTimeEntry.mutate(deleteEntryId, {
      onSuccess: () => { setShowDelete(false); setDeleteEntryId(null); },
    });
  };

  const openDeleteConfirm = (entryId: string) => { setDeleteEntryId(entryId); setShowDelete(true); };
  ...
  return (
    <div className="content-wrapper">
      ...
      <div className="content-grid">
        <div>
          <div className="mb-lg"><TimerWidget /></div>
          <Section title="Time Entries" action={
            <div className="section-tabs">
              <button className={`section-tab${tab === 'today' ? ' active' : ''}`} onClick={() => setTab('today')}>Today</button>
              <button className={`section-tab${tab === 'yesterday' ? ' active' : ''}`} onClick={() => setTab('yesterday')}>Yesterday</button>
              <button className={`section-tab${tab === 'weekly' ? ' active' : ''}`} onClick={() => setTab('weekly')}>This Week</button>
            </div>
          }>
            {tab !== 'weekly' ? (
              ...
              <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-tertiary)' }}>No time entries this week</div>
            ) : dayGroups.map((group) => (
      ...
      <Modal open={showManual} onClose={() => setShowManual(false)} title="Add Time Entry"
        footer={<><Button variant="secondary" onClick={() => setShowManual(false)}>Cancel</Button><Button variant="primary" onClick={handleManualEntry} disabled={logTime.isPending}>{logTime.isPending ? 'Saving...' : 'Save Entry'}</Button></>}
      >
        <div className="form-group"><label className="form-label">Task</label><select className="form-select" value={manualNodeId} onChange={(e) => setManualNodeId(e.target.value)}><option value="">Select a task...</option>{allNodes.map((n: Node) => (<option key={n.id} value={n.id}>{n.title}</option>))}</select></div>
        <div className="form-group"><label className="form-label">Duration (minutes)</label><input className="form-input" type="number" min="1" value={manualDuration} onChange={(e) => setManualDuration(e.target.value)} /></div>
        <div className="form-group"><label className="form-label">Notes (optional)</label><textarea className="form-input" placeholder="Add any additional notes..." value={manualNotes} onChange={(e) => setManualNotes(e.target.value)} /></div>
      </Modal>

      <Modal open={showDelete} onClose={() => { setShowDelete(false); setDeleteEntryId(null); }} title="Delete Time Entry"
        footer={<><Button variant="secondary" onClick={() => { setShowDelete(false); setDeleteEntryId(null); }}>Cancel</Button><Button variant="danger" onClick={handleDeleteEntry} disabled={deleteTimeEntry.isPending}>{deleteTimeEntry.isPending ? 'Deleting...' : 'Delete Entry'}</Button></>}
      >
        <p style={{ marginBottom: 16 }}>Are you sure you want to delete this time entry?</p>
