"""Backup commands for Taskyn CLI."""
...
from taskyn.config import get_database_path
...
def get_backup_dir() -> Path:
    ...
    backup_dir = Path.home() / ".taskyn" / "backups"
...
def create_backup(
    output: Optional[str] = typer.Option(
        None, "--output", "-o",
        help="Output path (default: ~/.taskyn/backups/taskyn_YYYYMMDD_HHMMSS.db)"
    ),
):
    ...
    if output:
        ...
        output_path = backup_dir / f"taskyn_{timestamp}.db"
...
def list_backups():
    ...
    backups = sorted(backup_dir.glob("taskyn_*.db"), reverse=True)
    ...
    for backup in backups:
        ...
        try:
            ts_str = backup.stem.replace("taskyn_", "")
...
def restore_backup(
    ...
    if db_path.exists():
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        pre_restore_backup = get_backup_dir() / f"taskyn_prerestore_{timestamp}.db"
